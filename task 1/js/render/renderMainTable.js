...
import deleteNote from '../func/deleteNote.js';
import archiveNoteItem from '../func/archiveNote.js';
import editNote from '../func/editNote.js';
...
export default function renderMainTable() {
  ...
  getNotes().map((note) => {
    if (!note?.archived) {
      mainTableBody.innerHTML += makeNoteItem(note);
    }
  });
